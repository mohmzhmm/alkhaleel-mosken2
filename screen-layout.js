(function(){
  ...
  const style=document.createElement('style');style.textContent='.alkhaleel-selectable{cursor:pointer!important;outline:4px solid #ffca28!important;outline-offset:3px!important;filter:drop-shadow(0 0 8px rgba(255,202,40,.75))}body.alkhaleel-selection-mode::before{content:"اضغط على العنصر الذي تريد تعديله";position:fixed;top:8px;left:50%;transform:translateX(-50%);z-index:99999;background:#ffca28;color:#102a27;padding:8px 18px;border-radius:999px;font:700 18px Tajawal,Arial,sans-serif;pointer-events:none;box-shadow:0 3px 14px #0008}';document.head.append(style);
  let selectionMode=new URLSearchParams(location.search).has('adminPreview'),highlighted=null;document.body.classList.toggle('alkhaleel-selection-mode',selectionMode);
  ...
  window.addEventListener('message',event=>{const data=event.data;if(data?.screenKey!==SCREEN_KEY)return;if(data.type==='alkhaleel-layout-preview')apply(data.settings);if(data.type==='alkhaleel-layout-select-mode'){selectionMode=Boolean(data.enabled);document.body.classList.toggle('alkhaleel-selection-mode',selectionMode);if(!selectionMode)highlight(null)}});
